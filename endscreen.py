...
class EndScreen:
    ...
    def display(self):
        """
        Display the end screen until the user clicks the Exit button or closes the window.
        ...
        while not self.done:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    # If the user clicks the 'x' button in the top right corner of the window, set done flag to True
                    self.done = True
                if event.type == pygame.MOUSEBUTTONDOWN:
                    # If the user clicks the mouse button, get the position of the click
                    x, y = event.pos
                    # If the position of the click is within the bounds of the Exit button, set done flag to True
                    if self.button_rect.collidepoint(x, y):
                        self.done = True
                        return True

            # Blit the end screen text, button background, and button text onto the game window
